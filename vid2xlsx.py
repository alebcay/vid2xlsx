...
workbook, palette, clt, labels, quant = xlsxwriter.Workbook(args["output"], {'constant_memory': True}), {}, None, None, None
...
while cap.more():
	...
	if count % args["frame"] == 0:
		...
		frame, (h, w), frame = cv2.resize(frame, (width, height), interpolation = cv2.INTER_CUBIC), frame.shape[:2], cv2.cvtColor(frame, cv2.COLOR_BGR2LAB)
		frame, clt, labels = frame.reshape((frame.shape[0] * frame.shape[1], 3), MiniBatchKMeans(n_clusters = args["colors"]), clt.fit_predict(frame)
		quant, quant, frame = clt.cluster_centers_.astype("uint8")[labels], quant.reshape((h, w, 3)), cv2.cvtColor(quant, cv2.COLOR_LAB2BGR)
